...
import com.oberasoftware.jasdb.api.storage.*;
...
public class DataBlockImpl implements DataBlock {
    ...
    public static final int STREAM_HEADER_SPACE = MemoryConstants.LONG_BYTE_SIZE;
    ...
    public DataBlockResult<byte[]> loadBytes(int offset) throws JasDBStorageException {
        ...
        if(result.getDataLength() < Integer.MAX_VALUE) {
            ...
            return new DataBlockResultImpl<>(result.getDataLength(), result.getEndBlock(), result.getNextOffset(), out.toByteArray());
        } else {
    ...
    @Override
    public DataBlockResult<ClonableDataStream> loadStream(long absolutePosition) throws JasDBStorageException {
        LOG.debug("Loading at absolute position: {}", absolutePosition);
        int relativeOffset = getRelativePosition(absolutePosition);
    ...
    @Override
    public DataBlockResult<ClonableDataStream> loadStream(final int offset) throws JasDBStorageException {
        ...
        if(available < STREAM_HEADER_SPACE) {
            byte[] valueBytes = new byte[MemoryConstants.LONG_BYTE_SIZE];
            mappedByteBuffer.position(offSetWithHeader);
            mappedByteBuffer.get(valueBytes, 0, available);
            int remaining = MemoryConstants.LONG_BYTE_SIZE - available;
    ...
    private DataBlockResult<ClonableDataStream> loadBlockStream(DataBlockImpl initialBlock, int offset, long dataLength) throws JasDBStorageException {
        ...
        if(((offset + dataLength) - DataBlockHeader.HEADER_SIZE) > capacity()) {
            ...
            return new DataBlockResultImpl<>(dataLength, dataBlockFactory.loadBlockForDataPosition(nextDataStream), dataStreamOffset, inputStream);
        } else {
            //inside same block
            return new DataBlockResultImpl<>(dataLength, initialBlock, ((int) (offset + dataLength)) - DataBlockHeader.HEADER_SIZE, inputStream);
        }
    }
    ...
    public DataBlockResult<Long> loadLong(int offset) throws JasDBStorageException {
        ...
        return new DataBlockResultImpl<>(result.getDataLength(), result.getEndBlock(), result.getNextOffset(), convertLong(result.getValue()));
    }
    ...
    public WriteResult writeDataStream(InputStream stream, boolean isDelegate) throws JasDBStorageException {
        ...
        if(available() > STREAM_HEADER_SPACE) {
            ...
            buffer.put(Arrays.copyOfRange(byteValue, available(), MemoryConstants.LONG_BYTE_SIZE));
    ...
    private byte[] convertLong(long value) {
        return ByteBuffer.allocate(MemoryConstants.LONG_BYTE_SIZE).putLong(value).array();
    }
